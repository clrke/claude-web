...
export async function syncPlanFromMarkdown(
  ...
  for (let i = 0; i < parsedSteps.length; i++) {
    ...
    if (existingStep) {
      // Step exists - check if content changed
  ...
  // Check for removed steps (steps in plan.json but not in markdown)
  for (const existingStep of currentPlan.steps) {
    if (!parsedStepIds.has(existingStep.id)) {
      result.removedCount++;
